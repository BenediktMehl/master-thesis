Ausgangslage:
Ich habe 6 verschiedene Visualisierungen (oder nur 5?)

Ich will herausfinden, wie gut diese Visualisierungen für verschiedene Aufgaben geeignet sind.

\section{Ablauf der Umfrage}

Einwilligung + Kurzbriefing: Ziel, Dauer, Datenschutz

Hier werde ich für Leute die keine Ahnung von Softwareentwicklung haben eine kurze Einführung geben, was Software ist, wie sie aufgebaut ist, was Metriken sind und wie sie dargestellt werden. 
...
\subsection{1. Meta Daten abfragen (Alter, Erfahrung mit Softwareentwicklung)}
Alter (in 10er schritten)
Erfahrung mit Softwareentwicklung:
Optionen: Keine, Informatik studiert, Beruflich Softwareentwicklung, Hobbymäßig Softwareentwicklung, Auftraggeber (PO, Stakeholder, bei Entwicklung dabei, aber kein entwickler)
Besteht eine Farbenblindheit? Ja/Nein -> wenn ja, welche


\subsection{2. Kurze Erlärung}
Was ist software, wie ist software aufgebaut
was Visualisiert wird, was die einzelnen Elemente sind, was die Metriken sind und wie sie dargestellt und interpretiert werden können.
erkläre die kombination aus verschiedenen metriken

\subsection{3. Testaufgaben}

Nehme ich echte Codebasen oder künstliche oder mix?
Ich würde einen Mix nehmen. Echte Codebasen, bei denen ich aber bestimmte Fehler bewusst einbaue, die Nutzer finden sollen. Sonst wird es schwer code basen zu finden, die alle dieser Probleme haben. 

Welche Metriken nehme ich?
Idee: Fläche: LOC, Höhe: Komplexität, Farbe: Testabdeckung

Option 1:
Ich zeige alle Visualisierungen gleichzeitig und frage, bei welcher Visualisierung sie die Aufgabe am besten lösen konnten.
Beispiel: In welcher der hier gezeigten Visualisierungen wird am deutlichsten, dass es im Ordner X viele schlechte Dateien gibt?

Option 2: 
Ich zeige nur eine Visualisierung und lasse die Nutzer die aufgaben lösen, dann frage ich sie, wie gut sie die aufgaben lösen konnten.
-> Problem: ich kann ja nicht immer die selbe Codebase zeigen oder weil die nutzer ja dazu lernen, das macht es schwerer zu vergleichen
Beispiel: Welcher Ordner hat die meisten schlechten Dateien?
Dann kann ich sowohl Korrektheit als auch Zeit messen, die sie brauchen.


Fragen:
A. Identifikation
Idee: Es gibt Ordner mit schlechter Testabdeckung, aber die Dateien sind nicht komplex. Es gibt Ordner mit hoher Komplexität, aber guter Testabdeckung. Es gibt dann auch mittel mäßig komplexe Dateien mit schlechter Testabdeckung.
A1: In welchem Ordner sind die Dateien mit der niedrigesten Testabdeckung?
A2: Welche Datei ist die größte?
A3: Welches ist die Datei mit der höchsten Komplexität?
A4: Finde Ordner mit vielen “Blobs” (interpretiere “Blobs” als Dateien mit schlechter Qualitätsfarbe Rot)

B. Aggregation
B1: In welchem Teil der Software sollten am dringensten Tests geschrieben werden? (Ordner mit den roten Dateien, die aber auch eine gewisse Komplexität haben)
B2: Welche Datei sollte am dringensten überarbeitet werden / in mehrere DAteien aufgeteilt werden und warum?

C. Struktur
C1: Liegen Datei X und Y im selben Ordner?
C2: Wie tief ist das maximale Nesting (hierarchische Tiefe)?
C3: Welche Datei ist am tiefsten verschachtelt?
C3: Gib den Pfad der Datei X an.
C4: Gibt es Dateien, die in andere Ordner verschoben werden sollten?

D. Muster/Anomalien
D1: Finde Ausreißer (Outlier): Welche Datei weicht deutlich in Höhe oder Farbe ab?
D2: Wie würdest du das beurteilen? 
D3: Finde Duplikate: Welche zwei Ordner sind wahrscheinlich Duplikate?

E. Qualitätbeurteilen
E1: Wie würdest du die Qualität dieser Codebasis anhand der Visualisierung einschätzen?
E2: An welchen Stellen der Software würdest du am ehesten mit Problemen rechnen und warum?


\subsection{4. Direkte Bewertung der Visualisierungen}
Selbe frage: Stelle ich die fragen pro Visualisierung oder im vergleich?


Wie viele Informationen konntest du aus diesem Layout entnehmen?
Wie gut konntest du die Struktur erkennen?
Wie gut wurde der Platz genutzt?

Wie Komplex war das Layout für dich?
Wie gut konntest du die Visualisierung verstehen?

Wie gut konntest du die Visualisierung auf große Codebasen anwenden?

Wie gut war die visuelle Metapher für dich?
Wie gut kontest du die abstrake visualisierung auf die Reele Welt übertragen?
Hilft es sich den Code als Stadt vorzustellen?



Am Ende auf jeden Fall Rangfolge:
Welche Visualisierung hat dir optimisch am besten gefallen?
Welche Visualisierung hat dir am meisten geholfen, Qualitätsprobleme zu finden?
Welche Visualisierung würdest du am ehesten in der Praxis verwenden?



Offene Fragen:
Was für Probleme hattest du mit den Visualisierungen?
...
%warum sind große files schlecht?
% 1. Sie sind schwer zu verstehen
% unerwartete Funktionalität
% 2. schwer zu navigieren
% single Responsibility Principle nicht eingehalten

%sage dass es in anlehnung an echte in der Praxis genutzte Software audits ist, die wegen daten schutz aber hier nicht gezeigt werden können
Zitat: "Our goal is to provide you with an overview of your codebase, identifying potential hotspots that will serve as a foundation for communication. We understand that code is not only developed by programmers but is also influenced by external requirements and factors that shape it over time."

" The resulting map resembles a small city featuring numerous colored rectangular cuboids, referred to as "buildings," each representing a file in your application. The height, area, and color of these buildings correspond to different metrics. Metrics that are shown in the maps, can be changed to fit different needs."

"Deep nesting in your code can significantly lower understanding and maintenance, leading to increased development time in the long run. As nesting levels increase, the complexity of interactions between different components grows, making it challenging to change. We consider a nesting level of 3(yellow) as medium and 4(red) or higher as high."


%in audits werden vergleiche gemacht mit verschiedenen metriken, es werden verschiede varianten von metrik kombinationen gezeigt, um verschiedene sachen zu zeigen, wenn man diesen technischen hintergrund weg nimmt geht es oft nur darum zu zeigen, wo was rot ist

Numerous files are either redundant or overly complex, which significantly complicates maintenance and
increases the likelihood of errors during updates or feature enhancements, in short it is hard to change.
Too many components exceed 500 lines of code and are burdened with an excessive number of comments
(beyond essential API specifications), numerous exception handlers, and high complexity levels.
Even parts of the code base that should help in simplifying interactions are hindered by their complexity,
making the codebase harder to understand, test, and modify.
Portions of the code are not utilizing modern Java features, limiting the ability to leverage modern language
enhancements that can improve performance and developer productivity.
Comments within the code suggest uncertainties about potential issues, highlighting areas that may be
fragile or prone to misinterpretation.
Although the legacy system is theoretically organized by conceptual file and folder structures, the presence
of numerous underlying issues makes refactoring and modification tasks exceptionally challenging.
The identified issues collectively pose substantial barriers to efficient maintenance, scalability, and
modernization of the software.

\subsection{Ideen für die Umfrage}

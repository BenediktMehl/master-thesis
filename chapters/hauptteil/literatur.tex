\chapter{Layout Auswahl} \label{sec:LiteraturRecherche}
Das Ziel dieses Abschnitts ist es 2D-Layouts zu finden, die geeignet sind, um in 2.5D extrudiert zu werden und Software-Qualitätsmetriken in Form der in Abschnit \ref{sec:Problemstellung} definierten Form in 2.5D zu visualisieren.
Im ersten Abschnitt stellen wir zunächst die Methodik vor, die wir bei der Suche der Layouts verwendet haben. Anschließend stellen wir die Ergebnisse der Suche und die finale entscheidung für die Layouts vor, die wir in dieser Arbeit implementieren und evaluieren werden.

\section{Methodik} \label{sec:Methodik}
Die ersten Schritte der hier verwendete Methodik orientiert sich an der Methodik der systematischen Literaturrecherche, wie sie in \textit{Procedures for performing systematic reviews}\cite{kitchenham2004procedures} beschrieben wird, ist aber leicht angepasst, da es vorallem um das finden und einordnen von Layouts geht und nicht direkt um das Bewerten und vergleichen von verschiedenen Arbeiten.
...
Es ist anzumerken, dass es wirklich unzählige verschiede Softwarevisualierungen gibt und niemals alle betrachtet und analysiert werden können. Deshalb konzentrieren wir uns auf die, die am relevantesten für unsere Problemstellung sind und die am relevantesten für die Forchung sind.
...
6. Doppelte Treffer filtern wir natürlich auch raus: 4 zB.: Visualization-based analysis of quality for large-scale software systems \cite{visbasedlarge}
...
Die anforderung die wir an das layout haben:
1. die input daten für das layout müssen die zuvor definierte form haben \ref{lst:nodeSchema}
sie müssen zunächst in einem layout schritt in die zuvor definierte form gebracht werden können: \ref{lst:nodeLayoutSchema}
Dann müssen sie in einem extrusions- und rendering-schritt sinnvoll in 2.5D form gebracht werden können

Sammeln und gruppieren der geeigneten verschiedener Layouts
wir gruppieren die layouts 
Wir unterscheiden bewusst zwischen Treemap und CodeCity, da CodeCity zwar ein Treemap Layout nutzt, dieses aber sehr untypisch für eigentliche Treemaps ist, da viele freie flächen gibt und nicht kante an kante liegt.

Danach werten wir aus und wählen dann layouts aus, die wir in dieser arbeit implementieren und vergleichen werden
...
\subsection{Google Scholar Suche} \label{sec:googleScholarSuche}

\subsubsection*{Ergebnisse der ersten Google Scholar Suche}

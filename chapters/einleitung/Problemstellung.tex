...
Die Darstellungen in der es in dieser Arbeit geht ist eine 2.5D-Darstellung, die als Grundlage ein 2D-Layout hat. Dieses 2D-Layout kann beliebig aussehen, mit formen, plazierungen, abständen, etc. Dabei haben alle Layouts ein gemeinsamen Schema (siehe Listing \ref{lst:nodeLayoutSchema}). 
...
Dieses Schema ermöglicht es, die Position und Größe jedes Knotens im 2D-Raum zu definieren. Die dritte Dimension wird durch eine zusätzliche Metrik extrudiert. Dabei muss diese Extrusion nicht zwingend linear sein. Zum Beispiel muss ein Kreis nicht zwingend zu einem Zylinder extrudiert werden, sondern kann auch zu einem Kegel oder einer Kugel extrudiert werden. Dadurch erhalten wir eine eindeutige Definition und Anforderung an 2D-Layouts und sind gleichzeitig flexibel in der Darstellung der dritten Dimension.

Im Mittelpunkt dieser Arbeit steht das zweidimensionale Layout der Knoten. Die Gestaltung dieses Layouts wird jedoch eingeschränkt, dadurch, dass Fläche, Höhe und Farbgebung (auch Schattierung, Struktur oder Transparenz) zur Darstellung verschiedener Metriken dienen. Maßnahmen wie das Einfärben von verschiedenen Ordnern oder Modulen, zur besseren visuellen Unterscheidung fallen für die Visualisierung in dieser Arbeit weg (bis auf wenige Ausnahmen - siehe Abschnitt \ref{sec:AbständeGeschwister}). Stattdessen wird der Fokus auf die geschickte Platzierung, Wahl von Abständen und Beschriftung gelegt. 
...
Lü und Fogarty identifizieren in ihrer Arbeit \cite{lu2008cascaded} ein zentrales Problem von Treemaps: Die erschwerte Wahrnehmbarkeit der zugrundeliegenden Hierarchiestruktur: \enquote{An important limitation of treemaps is the difficulty of discerning the structure of a hierarchy} \cite[1]{lu2008cascaded}

Dieses Problem wird bereits von Shneiderman und Johnson in ihrer Arbeit \cite{johnson1991tree} adressiert. Die Idee sind Abstände zwischen den Rechtecken, um die Hierarchieebene zu verdeutlichen. 
...
Das Grundproblem von der Erstellung von Treemaps, das Aufteilen von Rechtecken in kleinen rechtecke anhand deren Werte, ist bereits ohne Abstände NP-Hard \cite[3]{bruls2000squarified}, und zusätzliche Abstände verkomplizieren die Problematik noch weiter. Wesentliche Schwierigkeiten bei der Integration von Abständen in das Layout ergeben sich durch:
\begin{itemize}

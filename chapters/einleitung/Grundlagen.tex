...
function squarifyNode(parent: SquarifyNode) {
    ...
    // i ist der Index des Knotens, der gerade einsortiert wird
    while (i < numberOfChildren) {
        // Ein Schleifendurchlauf entspricht einer Reihe
        // Breite und Länge müssen nach jeder Reihe neu berechnet werden
        width = parent.x1 - x0
        length = parent.y1 - y0

        // minValue ist die Größe des kleinsten Knotens in der Reihe
        // maxValue ist die Größe des größten Knotens in der Reihe
        // sumValue ist die aktuelle Summe der Werte der Knoten in der Reihe
        minValue = maxValue = sumValue
        // alpha ist der feste Faktor der Reihe, mit dem das schlechteste Seitenverhältnis berechnet wird
        alpha = Math.max(length / width, width / length) / (value * aimedRatio)
        // beta ist der variable Faktor der Reihe
        beta = sumValue * sumValue * alpha
        // minRatio ist das aktuell schlechteste Seitenverhältnis der Reihe
        minRatio = Math.max(maxValue / beta, beta / minValue)

        // Füge neue Knoten hinzu, solange das Seitenverhältnis gleich bleibt oder sich verbessert
        for (; j < numberOfChildren; ++j) {
            ...
            beta = sumValue * sumValue * alpha
            // Berechne das neue schlechteste Seitenverhältnis
            newRatio = Math.max(maxValue / beta, beta / minValue)
            
            if (newRatio > minRatio) {
                // Wenn das neue Seitenverhältnis schlechter ist, breche die Schleife ab
                sumValue -= nodeValue
                break
            }
            minRatio = newRatio
        ...
        // Fülle nun die Reihe final mit den Knoten und setze die Koordinaten
        ...
        // Die größe des zu füllenden Bereichs wird um die Größe der Reihe verringert
        value -= sumValue
        i = j
    }
}

...
Softwarevisualisierung verfolgt das Ziel, komplexe Strukturen und Beziehungen in Softwaresystemen so darzustellen, dass sie schneller erfasst und verstanden werden können \cite{diehl2007software}. Visuelle Repräsentationen unterstützen nicht nur Entwickler bei der Analyse und Fehlersuche, sondern fördern auch die Kommunikation zwischen technischen und nicht-technischen Stakeholdern \cite{baltes2017sketches}.
...
\end{quote}
Anzumerken ist, dass die Autoren von CodeCity über 3D-Visualisierungen sprechen, jedoch in Wirklichkeit eine 2.5D-Visualisierung beschrieben wird. \cite{johnson1993treemapsPhd} 2.5D ist Laut Wikipedia \cite{wikipedia2.5D} eine Darstellung, die zwar drei Dimensionen nutzt, aber die dritte Dimension \enquote{deutliche Einschränkungen} aufweist, was bei CodeCity der Fall ist, da die Darstellung drei Dimensionen nutzt, aber die dritte Dimension \textit{nur} für die Extrusion der Gebäude genutzt wird und nicht zum beispiel für die Platzierung der Gebäude, was eine deutliche Einschränkung in der dritten Dimension darstellt.
